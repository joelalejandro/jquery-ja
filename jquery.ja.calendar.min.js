...
return dReturn;}})();var $newRow=function(){return $("<tr />");}
...
return i;return-1;}
var daysInMonth=new Date(selectedYear,selectedMonth+1,0).getDate();var weeksInMonth=getWeeksInMonth(selectedYear,selectedMonth);var firstDayOfMonth=new Date(selectedYear,selectedMonth,1);var firstWeekday=firstDayOfMonth.getDay();var renderDays=function()
...
{var $table=$("<table class='calendar' />");$.each($.fn.jaCalendar.header(settings,selectedYear,selectedMonth),function(i,obj){$(obj).appendTo($table);});$.each($.fn.jaCalendar.weekdays(weeksInMonth),function(i,obj){$(obj).appendTo($table);});$table.appendTo($target);renderDays();settings.month=selectedMonth+1;settings.year=selectedYear;$("button.prev-month",$target).click(function(){methods.prevMonth.apply($target);return false;});$("button.next-month",$target).click(function(){methods.nextMonth.apply($target);return false;});$.each($.fn.jaCalendar.footer(settings),function(i,obj){$(obj).appendTo($table);});$("button.today-button",$target).click(function(){methods.now.apply($target);return false;});$("input.year-input",$target).keypress(function(e){return"1234567890".indexOf(String.fromCharCode(e.keyCode))>-1;});$("button.go-period-button",$target).click(function(){methods.changePeriod.apply($target,[parseInt($(".calendar .month-list",$target).val())+1,$(".calendar .year-input",$target).val()]);return false;});$target.bind("selectedDateChanged",function(){if(typeof settings.selectedDateChanged==="function")
settings.selectedDateChanged.apply($target,[$target.data("selectedDate.jaCalendar"),settings.selectionMode]);});$target.bind("beforePeriodChange",function(){if(typeof settings.beforePeriodChange==="function")
...
$target.data("settings.jaCalendar",settings);},getDate:function()
...
{var fromDate=$calendar.data("selectedDate.jaCalendar").from;var $days=$("td.date",$calendar);var $firstDay=$("td.date.selected",$calendar);var fromIndex=$days.index($firstDay);var toIndex=$days.index($(this));if(fromIndex<0)fromIndex=0;var d=0;settings.maxRangeLength=settings.maxRangeLength==-1?toIndex:settings.maxRangeLength;if(toIndex-fromIndex<settings.minRangeLength-1&&settings.minRangeLength!=-1)
return;for(var i=fromIndex;i<=toIndex;i++)
{if(!$($days[i]).hasClass("selectable")||d>=settings.maxRangeLength)
{$calendar.data("selectedDate.jaCalendar",{from:fromDate,to:$($days[i-1]).data("date")});break;}
